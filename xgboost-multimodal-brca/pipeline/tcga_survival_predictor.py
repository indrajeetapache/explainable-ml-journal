...
class TCGASurvivalPredictor:
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.Series]:
        """Load and aggregate data to patient-level."""
        ...
        patient_df = aggregator.aggregate(df)
        
        # Drop rows missing target
        patient_df = patient_df[patient_df[self.target_column].notna()]
        ...
        logger.info(f"Target: {dict(zip(self.label_encoder.classes_, [n_neg, n_pos]))}")
